...
class ExternalBestTest(unittest.TestCase):
    ...
    def testBestExternalConfig(self):
        ...
        for router in list(SimBGP._router_list.values()) : 
            try : 
                self.assertTrue(router.bgp_mode==SimBGP.BGP_MODE_BEST_EXTERNAL)
            except AssertionError : 
                print("Router ", router.id, "is not best external :", router.bgp_mode, file=sys.stderr)
    ...
    def testBestExternalBasic(self):
        ...
        if "2.0/8" in SimBGP._router_list["0.0.1.3"].peers["0.0.1.1"].rib_in : 
